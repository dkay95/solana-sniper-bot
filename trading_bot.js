...
function fundChildWallets() {
  createdWallets.forEach(async (wallet) => {
    ...
    if (balance > 0.001 * LAMPORTS_PER_SOL) { // Ensure there's enough balance to cover fees
      const transaction = new Transaction().add(
        SystemProgram.transfer({
          fromPubkey: mainWallet.publicKey,
          toPubkey: walletKeypair.publicKey,
          lamports: 0.001 * LAMPORTS_PER_SOL, // Transfer 0.001 SOL to each child wallet
        })
      );

      try {
        const signature = await connection.sendTransaction(transaction, [mainWallet]);
        console.log(`Funded wallet ${wallet.publicKey} with signature: ${signature}`);
      } catch (error) {
...
function stopTrading() {
  ...
  // Implement logic to sell tokens and transfer back to main wallet
}
...
ws.on('message', function message(data) {
  ...
  if (tradingActive && eventData.mint === '3FMCAkk5yFz8U9u7j7zxUNjYnw8KZLxTxaDyo5jLpump') {
    createdWallets.forEach(async (wallet) => {
      const walletKeypair = Keypair.fromSecretKey(Uint8Array.from(wallet.secretKey.split(',').map(Number)));
...
// Example usage
fundChildWallets();
// Call startTrading() to begin trading
// Call stopTrading() to stop trading and transfer back to main wallet
